<?php
/**
 * CActiveRecord base class for classes that representing relational data.
...
{	
    ...
    */
    private function createObjectFromTable()
    {
        ...
        foreach($cols as $array){
            ...
            $this->_columnTypes[$array[0]] = array_shift(explode('(', $array[1]));
            if($array[3] == 'PRI'){
    ...
    public function save()
    {
		...
		if($this->beforeSave($this->_pkValue)){
            foreach($this->_columns as $column => $val){
                $relations = $this->getRelations();                
                if($column != 'id' && $column != $this->_primaryKey && !in_array($column, $relations['fieldsArray'])){  //  && ($column != 'created_on') && !$NEW)
    ...
    */
	...
	public function count($conditions = '', $params = '')
	{
        ...
        $sql = 'SELECT COUNT(*) as cnt FROM `'.CConfig::get('db.prefix').$this->_table.'` '.$whereClause.' LIMIT 1';
        $result = $this->db->select($sql, $params);
