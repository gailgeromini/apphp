...
class CMailer
{
    ...
    public static function smtpMailer($to, $subject, $message, $params = '')
    {
		...
		$mail->SMTPSecure = CConfig::get('email.smtp.secure') ? 'ssl' : '';      // sets the prefix to the server
		$mail->Host       = CConfig::get('email.smtp.host');
		$mail->Port       = CConfig::get('email.smtp.port');
		$mail->Username   = CConfig::get('email.smtp.username');
		$mail->Password   = CConfig::get('email.smtp.password');
